...
=
\begin{pmatrix}
\bm{x}_{\mathrm{t}}^\transp(\bm{q}) & \bm{x}_{\mathrm{r}}^\transp(\bm{q})
\end{pmatrix}^\transp
=
\bm{f} (\bm{q})
\label{equ:dirkin}
\end{equation}  
%
describing the end-effector frames position 
%
\begin{equation}
\bm{x}_{\mathrm{t}} (\bm{q})
=
\ortvek{0}{r}{}{E} (\bm{q})
\label{equ:xt_def}
\end{equation}  
...
%
\begin{equation}
\rotmat{0}{E} (\bm{x}_{\mathrm{r}} (\bm{q}))
=
\begin{pmatrix}\vek{n}{}{E} & \vek{o}{}{E} & \vek{a}{}{E}\end{pmatrix}
\end{equation}  
% 
with respect to the base frame $\ks{0}$ expressed without loss of generality in a set $\bm{\beta}$ of $X$-$Y$-$Z$-Euler angles\footnote{other sets of Euler angles with the same properties may be used as well}
%
\begin{equation}
\bm{x}_{\mathrm{r}}
=
...
\end{equation}
%
In the following, a general pose $\bm{x}$ of the robot end-effector is regarded independent of the forward kinematics $\bm{f}(\bm{q})$ in (\ref{equ:dirkin}).
The general pose is the desired end-effector pose in the inverse kinematics problem and will be termed with ``$\bm{x}$'' without further supplements for the sake of compactness of the equations.
The deviation $\bm{\Phi}$ between the desired end-effector frame $\ks{D}$ expressed with $\bm{x}$ and the actual robots end-effector frame $\ks{E}$ expressed with $\bm{f}(\bm{q})$ with
%
\begin{equation}
...
\end{equation}
%
is the vector from $\ks{D}$ to $\ks{E}$.
The rotational part
%
\begin{align}
...
Using this convention an adaption to tasks with functional redundancy is only possible with some of the referenced state of the art methods described in Sec.\,\ref{sec:Intro}.
%
\begin{figure}[tb]
...
%\begin{align}
%\rotmat{D}{E}(\bm{x},\bm{q}) 
%& = \rotmat{0}{D}^\transp (\bm{x})\rotmat{0}{E}(\bm{q}) \\
%&={}_{(Ex)}\begin{pmatrix}
%\vek{n}{}{E}(\bm{x}_{\mathrm{r}}) & \vek{o}{}{E}(\bm{x}_{\mathrm{r}}) & \vek{a}{}{E}(\bm{x}_{\mathrm{r}})
%\end{pmatrix}
%\end{align}

It is apparent, that using $\bm{\alpha}$ as defined in the previous subsection\,\ref{sec:RecEulAng_KinDesc}, all three components of $\bm{x}_{\mathrm{r}}=\bm{\beta}$ affect the three unit vectors of the rotation matrix
%
\begin{equation}
\rotmat{0}{D} (\bm{x}_{\mathrm{r}})
=
\begin{pmatrix}\vek{n}{}{D} & \vek{o}{}{D} & \vek{a}{}{D}\end{pmatrix}.
\end{equation}  
% 
...
\end{equation}
%
equal to (\ref{equ:Phit_def}) and the rotational part
%
\begin{align}
...
\end{pmatrix}^\transp \nonumber \\
&=
\begin{pmatrix}\bm{0}_{2 \times 1} & \bm{1}_{2 \times 2}\end{pmatrix} \bm{\Phi} (\bm{q},\bm{x}) \nonumber \\
&= \begin{pmatrix} \bm{0} & \bm{1}\end{pmatrix} \bm{\alpha}\left(\rotmat{0}{A1}^\transp (\bm{\eta}_{\mathrm{r}})\rotmat{0}{E}(\bm{q})\right) \in {\mathbb{R}}^{2}
\end{align}
%
adapted from (\ref{equ:Phir_def}) by using (\ref{equ:alpha_dep_beta} and (\ref{equ:etar_def}).

The condition $\bm{\Phi}=\bm{0}$ or $\bm{\Psi}=\bm{0}$ leads to a valid configuration of positioning the end-effector and the complete orientation of the end-effector (using $\bm{\Phi}$) or the orientation only of the tool-axis (using $\bm{\Psi}$).
Following \cite{GoldenbergBenFen1985}, the inverse kinematics problem for serial link robots with 6-DoF at the step $k+1$ can be derived with the Taylor series of $\bm{\Phi}(\bm{q},\bm{x})$ to
%
\begin{equation}
...
We define the column operator $\overline{\bm{R}}$ for matrices with
%
\begin{equation}
\bm{R}=\begin{pmatrix}
\bm{n} & \bm{o} & \bm{a}
...
\left(\frac{\partial \overline{\prod}\left(\rotmato{0}{D}^\transp, \rotmato{0}{E}\right)}{\partial \rotmato{0}{E}}
\right)
\left(\frac{\partial \rotmato{0}{E}(\bm{q})}{\partial \bm{q}} \right).  \nonumber
\end{align}
...
The last partial derivative $\partial \rotmato{0}{E}/\partial \bm{q}$ can be derived efficiently with computer algebra systems.
...
&=
\frac{\partial}{\partial \rotmato{0}{N}} \overline{\prod}\left( \rotmato{0}{N}, \rotmato{N}{E}\right)
...
In the following, the definitions for $\bm{x}$ still hold from  (\ref{equ:xt_def}),\,(\ref{equ:xr_def}), but they now refer to the base frame $\ks{0}$ and the end-effector frame $\ks{D}$ of the whole PKM instead just of single kinematic chains.
...
An end-effector orientation $\bm{\beta}=\bm{x}_{\mathrm{r}}$ gives the rotation matrix from\footnote{The matrix rotates vectors from $\ks{E}$ to $\ks{D}$} the actual end-effector frame $\ks{E}$ to the desired frame end-effector $\ks{D}$
%
\begin{equation}
...
With $\overline{\bm{R}}=[n_x,n_y,n_z ,o_x,o_y,o_z,a_x,a_y,a_z]^\transp$ the gradient in (\ref{equ:grad_Phi_q}) becomes
