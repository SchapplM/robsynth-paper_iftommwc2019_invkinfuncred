...
\end{equation}
%
The Euler angles can be calculated from a rotation matrix in the same way as before\footnote{Utilizing the sign-aware operator $\arctantwo(y,x)$ instead of $\arctan(y/x)$ allows angles to be in $(-\pi,+\pi]$ and removes ambiguities in the solution.} using these operators with the notation
%
\begin{equation}
