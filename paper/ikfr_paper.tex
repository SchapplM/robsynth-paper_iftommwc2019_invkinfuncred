...
Since the first papers in the 1980s the fields of inverse kinematics \cite{GoldenbergBenFen1985} and the resolution of intrinsic redundancy \cite{Yoshikawa1984} in robotics have been extensively elaborated upon.
Inverse kinematics for robot manipulators can be approached with gradient-based numeric approaches on the level of joint positions and velocity.
Due to the nonlinearity of rotation on position level \cite{GoldenbergBenFen1985}, the latter is usually preferred.

The kinematics of robots performing tasks with less than five instead of six degrees of freedom (DoF) has drawn much less attention, even if many industrial relevant tasks only require 5-DoF such as the class of 3T2R-tasks where the task rotation is axis-symmetric.
...
Augmenting the joint space is possible by inserting an additional joint for the rotation around the axis of symmetry (``tool axis'') \cite{Baron2000}.
...
The choice of performance indices aims to improve the task execution with measures such as quadratic \cite{HuoBar2005} or hyperbolic \cite{ZhuQuCaoYan2013} distance from the joint limits, singularity avoidance via Frobenius-norm condition number \cite{ZhuQuCaoYan2013}, squared condition number \cite{LegerAng2016} or a combination of manipulability and condition number \cite{HuoBar2008} or stiffness \cite{GuoDonKe2015}.

The characteristic length required to normalize the Jacobian for the performance indices can be either chosen constant regarding the robot geometry \cite{ZhuQuCaoYan2013} or as an optimization parameter \cite{LegerAng2016}.
...
By this definition, the rotation component belonging to the symmetrical tool axis in 3T2R tasks is always the same for the absolute orientation and for the orientation error.
The effect of the tool axis can be eliminated from the kinematic equations.
...
\begin{itemize}
    ...
    \item an application leading to an efficient solution for inverse kinematics with functional redundancy
...
Simulative results for the inverse kinematics of serial robots in 3T2R tasks is presented in Sec.\,\ref{sec:SimEvalSerRobIK} to support the presented approach.
